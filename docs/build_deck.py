...
text(s, 0.7, 6.55, 11.9, 0.5, [[('Наш опрос учеников (n = 13): ', {'bold': True, 'color': BLUE}),
                              ('77% сказали, что «иногда» или «редко» понимают, почему ошиблись в пробнике.', {'color': INK})]], size=13)
notes(s, 'Цифры сверху проверены по первоисточникам (README → «Источники»): OECD PISA 2022 Country Note: Kazakhstan; ЕНТ-2025 — Вечерняя Астана, 02.06.2025. Нижняя строка — наш опрос, n=13: только ответы с правдоподобными по времени отметками. Ещё 30 ответов пришли за 49 секунд подряд и не используются — подробности в docs/validation-plan.md.')
...
text(s, 0.7, 5.45, 11.9, 0.55, 'Полнота и точность — контрольная симуляция: 1000 новых виртуальных учеников, не использованных при доработке (было 79% и 72% до правила «перевес в 2»). Модель ученика наша, поэтому это оценка сверху. Покрытие — сверка 17 тем с официальной спецификацией НЦТ (с 2026 г.).',
     size=12, color=MUTED)
shape(s, MSO_SHAPE.RECTANGLE, 0.7, 6.05, 11.9, 1.1, BLUE_PALE, BLUE, 1.5)
text(s, 0.95, 6.14, 11.4, 0.95, [
    [('Проверено на людях: ', {'bold': True, 'color': BLUE}),
     ('интервью с 4 преподавателями (учитель математики, репетитор, преподаватель учебного центра, учитель физики).', {'color': INK})],
    [('«Ученик не может решать квадратные уравнения, а проблема — в отрицательных числах, дробях или раскрытии скобок» — учитель математики. ', {'italic': True, 'color': INK}),
     ('Ручная диагностика занимает от одного занятия до недели; Tamyr — 10–34 вопроса.', {'bold': True, 'color': INK})]
], size=12.5, spacing=3)
notes(s, 'Говорите честно про симуляцию, затем — что реально проверили: 4 интервью (docs/validation-plan.md). Опрос учеников собран, но временные метки ответов не проходят проверку на подлинность — не выносите его на слайд, пока не подтверждено, как именно он собирался.')
...
cols = [('Аудитория', ['216 тыс. абитуриентов ЕНТ-2025', '≈184 тыс. сдают обязательную мат. грамотность (наша оценка: без творческих специальностей)', '72% сдают ЕНТ на казахском → казахская версия — шаг №1']),
        ...
        ('Деньги', ['По интервью (n=4): репетитор — 5–10 тыс ₸/мес', 'Учебный центр: 30–70 тыс ₸/мес, либо 500–1 500 ₸/ученика', 'Школа: бесплатно для учителя ради охвата (гипотеза)'])]
for i, (h_, items) in enumerate(cols):
    ...
    bullets(tb)
text(s, 0.7, 6.9, 9.0, 0.3, 'Источники: НЦТ (формат ЕНТ); Вечерняя Астана, 02.06.2025; интервью с 4 преподавателями, сентябрь 2026', size=10, color=MUTED)
footer(s)
notes(s, 'Цены репетитора и центра — из 2 реальных интервью (не гипотеза), см. docs/validation-plan.md. Школьная цена — всё ещё гипотеза, честно так и скажите. 184 тыс. — оценка: 216 тыс. минус около 15% творческих специальностей.')
