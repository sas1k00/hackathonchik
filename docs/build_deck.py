...
bg(s, BG)
brand(s)
heading(s, 'ПРИЧИНЫ', 'ПОЧЕМУ ЭТО ПРОИСХОДИТ')
cards = [('Математика кумулятивна', 'Каждая тема держится на предыдущих. Пробел в базовой теме всплывает позже — в теме, которая на ней держится.'),
         ('Тесты измеряют балл', 'Пробники и контрольные говорят, где ошибка произошла, но не почему. Выбранный неверный вариант не анализируется.'),
         ('У учителя нет времени', 'В классе 25–30 учеников. Индивидуально проверить каждого по всем темам с 5 по 9 класс невозможно.')]
for i, (h_, b) in enumerate(cards):
    x = 0.7 + i * 4.1
    shape(s, MSO_SHAPE.RECTANGLE, x, 3.0, 3.8, 3.05, WHITE, LINE, 1)
    hx = hexagon(s, x + 0.3, 2.62, 0.95, BLUE, WHITE, 3)
    label(hx, str(i + 1), size=22, font=DISPLAY)
    text(s, x + 0.3, 3.75, 3.2, 0.8, h_, size=19, bold=True, color=BLUE)
    text(s, x + 0.3, 4.5, 3.2, 1.5, b, size=15, color=GRAY)
text(s, 0.7, 6.35, 11.9, 0.6, [[('Место для вашей цифры из опроса: ', {'bold': True, 'color': RED}),
                              ('«__% учеников не понимают, почему ошиблись в пробнике» (n = __). Заполните по docs/validation-plan.md или удалите строку.', {'color': GRAY})]], size=13)
dots_cluster(s, [(12.2, 1.95, 0.14), (12.65, 2.3, 0.1), (11.8, 2.4, 0.1)])
notes(s, 'Три причины. Если провели опрос, назовите реальную цифру; если нет, удалите нижнюю строку.')
...
stats = [('79%', 'настоящих корней найдено\n(полнота)'), ('72%', 'найденных корней верны\n(точность)'),
         ('9 / 28', 'вопросов: без пробелов /\nс одним пробелом'), ('16/16', 'юнит-тестов\nпроходят')]
...
text(s, 0.7, 5.45, 11.9, 0.55, 'Контрольная симуляция: 1000 новых виртуальных учеников с известными пробелами, не использованных при доработке движка. Модель ученика наша, поэтому это оценка сверху; без шума алгоритм находит 100% корней.',
     size=12, color=MUTED)
...
# =====================================================================
# 10. Внедрение и модель
# =====================================================================
s = prs.slides.add_slide(BLANK)
bg(s, BG)
brand(s)
heading(s, 'ВНЕДРЕНИЕ', 'РЕАЛИЗУЕМО УЖЕ СЕЙЧАС')
