...
text(s, 0.7, 2.0, 5.6, 3.8, [
    [('Пример: ', {'bold': True, 'color': GREEN}), ('ученица готовится к ЕНТ и ошибается в квадратных уравнениях.', {'bold': True})],
...
cards = [('Математика кумулятивна', 'Каждая тема держится на предыдущих. Пробел в базовой теме всплывает позже — в теме, которая на ней держится.', GREEN),
...
steps = [('Адаптивная диагностика', 'Начинаем со сложной темы и спускаемся по графу из 17 тем только там, где есть ошибки. Без пробелов — около 9 вопросов.'),
...
s = prs.slides.add_slide(BLANK)
bg(s, WHITE)
title(s, 'Идея не новая — новое в связке и в месте', 'Похожие решения')
...
text(s, 3.2, 1.95, 4.3, 0.35, 'ЧТО ДЕЛАЕТ', size=11, color=INK2, bold=True)
text(s, 7.9, 1.95, 4.7, 0.35, 'ЧЕМ ОТЛИЧАЕТСЯ TAMYR', size=11, color=GREEN, bold=True)
for i, (n, org, what, diff) in enumerate(rows):
    y = 2.35 + i * 0.98
    box(s, 0.7, y, 11.9, 0.86, LIGHT, radius=0.1)
    text(s, 0.95, y + 0.13, 2.1, 0.35, n, size=17, bold=True, font=HEAD)
    text(s, 0.95, y + 0.5, 2.1, 0.3, org, size=11, color=INK2)
    text(s, 3.2, y + 0.1, 4.4, 0.66, what, size=13, color=INK, anchor=MSO_ANCHOR.MIDDLE)
    text(s, 7.9, y + 0.1, 4.5, 0.66, diff, size=13, color=GREEN, bold=True, anchor=MSO_ANCHOR.MIDDLE)
text(s, 0.7, 6.4, 11.9, 0.6, [[('Наше отличие: ', {'bold': True}), ('неверный вариант → домашняя тема ошибки → проверка корня, под программу Казахстана, на любом телефоне, с панелью учителя.', {'color': INK2})]], size=15)
s.notes_slide.notes_text_frame.text = 'Не утверждайте, что идея уникальна: жюри может знать ALEKS и Eedi. Перед защитой проверьте актуальные описания конкурентов на их сайтах.'

# ---------- 9. Доказательства ----------
s = prs.slides.add_slide(BLANK)
bg(s, DARK)
title(s, 'Что уже проверено', 'Валидация', color=WHITE, kcolor=MINT)
stats = [('79%', 'настоящих корней\nнайдено (полнота)'), ('72%', 'найденных корней\nверны (точность)'), ('9 / 28', 'вопросов: без пробелов /\nс одним пробелом'), ('16 / 16', 'юнит-тестов\nпроходят')]
...
text(s, 0.7, 4.35, 11.9, 0.7, 'Контрольная симуляция: 1000 новых виртуальных учеников с известными пробелами, не использованных при доработке движка. Угадывание 20%, ошибки в знакомых темах 8%. Модель ученика наша, поэтому это оценка сверху; без шума алгоритм находит 100% корней.',
     size=14, color=RGBColor(0xB5, 0xCC, 0xC3))
...
cols = [('Технологии', ['HTML/CSS/JS без зависимостей', 'Работает офлайн на любом телефоне', 'Граф и банк заданий — данные: новый предмет = новый файл', 'Персональные данные остаются на устройстве']),
        ...
        ('Для запуска', ['Проверка графа 2–3 учителями', 'Банк 10+ вопросов на тему', 'Казахская версия', 'Пилот в 2–3 классах'])]
...
road = [('Сейчас', 'MVP: 17 тем, 102 задания, 43 ошибки мышления, панель учителя'), ('1 месяц', 'Пилот в 2–3 классах, казахский язык'),
        ('3 месяца', 'Байесовская модель освоения, 10+ вопросов на тему'), ('6 месяцев', 'Геометрия и мат. грамотность ЕНТ, вход по коду класса')]
